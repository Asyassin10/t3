    ...
    <div>
        <div class="py-4">
                        ...
                        <tr>
                            <td class="w-1/2 align-top">
                                <div class="text-sm text-neutral-600">
                                    <p class="font-bold">{{ __('app.client') }} : {{ $facture["clientB2B"]->client_b2b_name }} </p>
                                    @foreach ($facture["client_commercial_data"] as $client_commercial_data)
                                        <p>{{ $client_commercial_data->key }}: {{ $client_commercial_data->value }}
                                        </p>
                            ...
                            <td class="w-1/2 align-top text-right">
                                <div class="text-sm text-neutral-600">
                                  {{-- -<p class="font-bold">Customer Company</p> --}}
                                    @foreach ($facture["client_esoft_commercial_data"] as $client_esoft_commercial_data)
                                        <p>{{ $client_esoft_commercial_data->key }}: {{ $client_esoft_commercial_data->value }}
                                        </p>
                ...
                <table class="w-full border-collapse border-spacing-0">
                    <thead>
                        <tr>
                            ...
                            <td class="border-b-2 border-main pb-3 pl-2 font-bold text-main">{{ __('app.user') }}</td>
                            <td class="border-b-2 border-main pb-3 pl-2 font-bold text-main">{{ __('app.role') }}</td>
                            <td class="border-b-2 border-main pb-3 pl-2 text-right font-bold text-main">{{ __('app.daily_rate') }}</td>
                            <td class="border-b-2 border-main pb-3 pl-2 text-left font-bold text-main">{{ __('app.project') }}</td>
                            <td class="border-b-2 border-main pb-3 pl-2 text-left font-bold text-main">{{ __('app.activity') }}</td>
                            <td class="border-b-2 border-main pb-3 pl-2 text-center font-bold text-main">{{ __('app.quantity') }}</td>
                            <td class="border-b-2 border-main pb-3 pl-2 text-center font-bold text-main">{{ __('app.price') }}
                            </td>
                        ...
                        @foreach ($facture['data'] as $fac)
                            <tr>
                                ...
                                <td class="border-b py-3 pl-2 text-right">
                                    {{ $fac->user->price_per_day }}
                                </td>
                                <td class="border-b py-3 pl-2 text-right">
                                    {{ $fac->project->project_name }}
                                </td>
                                <td class="border-b py-3 pl-2 text-right">
                                    {{ $fac->activite->activity_name }}
                                </td>
                                <td class="border-b py-3 pl-2 text-center">
                                    {{ $fac->count_of_days }}
                                </td>
                                <td class="border-b py-3 pl-2 text-center">
                                    €{{ $fac->user->price_total }}
                                </td>
                                                    ...
                                                    <tbody>
                                                        <tr>
                                                            <td class="border-b p-3">
                                                                <div class="whitespace-nowrap text-slate-400">{{ __('app.net_total') }}:
                                                                </div>
                                                            </td>
                                                        ...
                                                        <tr>
                                                            <td class="p-3">
                                                                <div class="whitespace-nowrap text-slate-400">{{ __('app.vat') }} (20 %):
                                                                </div>
                                                            </td>
                                                            <td class="p-3 text-right">
                                                                <div class="whitespace-nowrap font-bold text-main">
                                                                    €{{ $facture['total_tva'] }}</div>
                                                            </td>
                                                        </tr>
                                                        <tr>
            ...
            <div class="px-14 text-sm text-neutral-700">
                <p class="text-main font-bold">{{ __('app.payment_details') }} -

                @if (is_null($facture["facture"]->paid_at))
                    {{ __('app.unpaid') }}
                @else
                    {{ __('app.paid_on') }} {{ \Carbon\Carbon::parse($facture["facture"]->paid_at)->format('d/m/Y') }}
                @endif
                </p>
                @foreach ($facture["payments"] as $payment)
                    <p>{{ $payment->amount }} - {{ $payment->payment_method }}</p>
                @endforeach

            </div>

            <div class="px-14 py-10 text-sm text-neutral-700">
                <p class="text-main font-bold">{{ __('app.notes') }}</p>
                <p class="italic">
                    @if (is_null($facture["facture"]->note))
                      --
                    @else
                           {{ $facture["facture"]->note }}
                    @endif
                </p>
                </dvi>
